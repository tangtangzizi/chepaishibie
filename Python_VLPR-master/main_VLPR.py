...
class Surface(ttk.Frame):
    ...
    def __init__(self, win):
        ...
        win.title("车牌识别与车牌号码提取")
        win.state("zoomed")
        self.pack(fill=tk.BOTH, expand=tk.YES, padx="10", pady="10")
        frame_left.pack(side=LEFT, expand=1, fill=BOTH)
        frame_right1.pack(side=TOP, expand=1, fill=tk.Y)
        frame_right2.pack(side=RIGHT, expand=0)
        ttk.Label(frame_left, text='图片：').pack(anchor="nw")
        ...
        from_vedio_ctl = ttk.Button(frame_right2, text="选择拍照", width=20, command=self.from_vedio)
        from_img_pre = ttk.Button(frame_right2, text="形状预处理图像", width=20, command=self.show_img_pre)
